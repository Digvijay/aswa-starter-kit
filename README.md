...
| `/api/status` works right after deploy then later returns 503 / `Backend call failure` / `Cosmos … blocked by your … firewall settings` | Some tenants run security automation (e.g. Defender for Cloud auto-remediation, custom policies) that flips `publicNetworkAccess` to `Disabled` on the storage account and Cosmos account a few minutes after deploy, even though the Bicep declares `Enabled`. Verify with `az storage account show -n <stg> -g <rg> --query publicNetworkAccess` and `az cosmosdb show -n <cosmos> -g <rg> --query publicNetworkAccess`. Re-enable manually with `az storage account update -n <stg> -g <rg> --public-network-access Enabled --default-action Allow` and `az cosmosdb update -n <cosmos> -g <rg> --public-network-access Enabled`. The durable fix on locked-down tenants is to add a VNet + private endpoints for storage and Cosmos and put the Function App on a delegated subnet — out of scope for this minimal starter. |
